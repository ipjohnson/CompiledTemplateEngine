...
public class TemplateImplementationGenerator {
    ...
    public TemplateImplementationGenerator(CancellationToken cancellationToken) {
        _cancellationToken = cancellationToken;
    }

    public void GenerateImplementation(ClassDefinition classDefinition, ModuleEntryPointModel entryModel,
        TemplateModel templateModel) {
        var executeMethodDefinition = ExecuteMethodDefinition(classDefinition);
        var templateNodes = templateModel.TemplateActionNodes;
        ...
        executeMethodDefinition.AddCode(
            "var executionContext = new {arg1}({arg2}, serviceProvider, requestValue, _services, _templateExecutionService, _stringEscapeService, writer, customData, parentTemplateExecutionContext);",
            KnownTemplateTypes.Engine.TemplateExecutionContext, templateModel.TemplateExtension);
        ...
        var startingNode =
            ProcessModelUsingAndInjectStatements(classDefinition, executeMethodDefinition, templateNodes);
    ...
    private static MethodDefinition ExecuteMethodDefinition(ClassDefinition classDefinition) {
        ...
        methodDefinition.AddParameter(typeof(ImmutableDictionary<string,object>), "customData");
        methodDefinition.AddParameter(typeof(IServiceProvider), "serviceProvider");
        methodDefinition.AddParameter(KnownTemplateTypes.Interfaces.ITemplateOutputWriter, "writer");
        methodDefinition.AddParameter(KnownTemplateTypes.Interfaces.ITemplateExecutionContext,
            ...
            while (processingInjectStatement) {
                if (templateActionNodes[startIndex].ActionText == "inject") {
                    ...
                    if (injectNode.ArgumentList.Count == 2) {
                        ...
                        methodDefinition.AddUsingNamespace(
                            "KnownTypes.Microsoft.DependencyInjection");
                    }
    ...
    private static string GetCsharpStatement(GenerationContext context, TemplateActionNode actionNode) {
        ...
        if (csharpStatement.StartsWith("^")) {
            ...
            context.InvokeMethod.AddUsingNamespace(KnownTemplateTypes.Interfaces.Namespace);

            var propertyStatement = csharpStatement.Substring(1);

            csharpStatement =
                $"new PropertyValue({context.CurrentModel.Name}.{propertyStatement}, \"{propertyStatement}\")";
        }
        else {
    ...
    private void ContentActionNode(GenerationContext context) {
        ...
        if (!string.IsNullOrEmpty(context.CurrentNode.FieldName)) {
            ...
            context.CurrentBlock.AddCode($"// no field name provided");
        }
    }
